...
bool RRBotController::init(hardware_interface::PositionJointInterface * hw, ros::NodeHandle & nh)
{
  ...
  for(size_t i = 0; i<num_joints; ++i)
  {
    ...
    {
      ROS_ERROR_STREAM_NAMED("RRBotController", "Exception thrown: " << e.what());
      return false;
    }
  }
...
void RRBotController::starting(const ros::Time & /*time*/)
{
...
void RRBotController::update(const ros::Time & /*time*/, const ros::Duration & /*period*/)
{
  std::vector<double> & commands = *commands_buffer_.readFromRT();
  for(size_t i = 0; i < joint_names_.size(); ++i) {
...
{
  if (msg->displacements.size() != joint_names_.size())
  {
    ROS_ERROR_STREAM_NAMED("RRBotController",
      "Dimension of command (" << msg->displacements.size() << ") does not match number of joints ("
